...
uint8_t extern  pre_ams_imd_error;
...
void can_rx_routine(void){
    ...
    if (can_rx_flag == 1){
        ...
        mcb_tlb_battery_shut_status_unpack(&tlb_shut_rx, &can_buffer[1].data, 4);
        ...
        if (can_buffer[0].data_present == 1){
            ...
            hvcb_hvb_rx_v_cell_unpack(&v_cell_rx, &can_buffer[0].data, sizeof(can_buffer[0].data));
...
if(can_send(&hcan1, &tlb_battery_shut_buffer, &TxHeader, CAN_RX_FIFO0) != HAL_OK) Error_Handler();
...
hvcb_hvb_rx_v_cell_pack(&v_cell_buffer, &v_cell, sizeof(v_cell));


TxHeader.DLC = sizeof(v_cell_buffer);
TxHeader.StdId = v_cell_id;

if(can_send(&hcan1, &v_cell_buffer, &TxHeader, CAN_RX_FIFO0)!= HAL_OK) Error_Handler();
