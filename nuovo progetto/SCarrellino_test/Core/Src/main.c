...
//invio messaggio can
if (HAL_CAN_AddTxMessage(&hcan1, &TxHeader, (uint8_t *)dati, &txmailbox) != HAL_OK){
...
log = HAL_CAN_GetRxFifoFillLevel(&hcan1, rxFIFO);
HAL_UART_Transmit(&huart2, (uint8_t *)"log della rx : ", strlen(""),HAL_MAX_DELAY);
HAL_UART_Transmit(&huart2, (uint8_t *)log, sizeof(log),HAL_MAX_DELAY);


if (HAL_CAN_GetRxMessage(&hcan1, rxFIFO, &RxHeader, datirx) != HAL_OK){
  HAL_UART_Transmit(&huart2, (uint8_t *)"errore in ricezione CAN", strlen("errore in ricezione CAN"),HAL_MAX_DELAY);
  Error_Handler();
}
